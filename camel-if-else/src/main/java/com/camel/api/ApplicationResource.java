...
public class ApplicationResource  extends RouteBuilder{
	...
	public void configure() throws Exception {
		...
		from("file:D:\\Json").to("direct:condition");
		...
		from("direct:condition")
         ...
         .log("Body is ${body}")
         .choice()
         	.when(simple("${body.getMarks()} > 30"))
         	.choice()
         		.when(simple("${body.getMarks()} > 40"))
         		.log("${body.getName()} is average")
         		.otherwise()
         		.log("${body.getName()} is passed")
         		.endChoice()
         	.otherwise()
         	.log("${body.getName()} is failed")
         	.end();
		//.to("");
	}
	}
